...
##fjr read data
    ...
    while(bLow > bHigh + 2*tolerance):
        ...
        for i in range(0, m):         
            ...
            if((alpha[i] > 0 and alpha[i] < cost) or (label[i] > 0 and alpha[i] ==0) or (label[i]<0 and alpha[i] == cost)):
                tempF1[i] = devF[i];
            else:
                tempF1[i] = float('Inf');

            #if(((label[i] > 0) and (alpha[i] > epsilon)) or ((label[i] < 0) and (alpha[i] < cEpsilon))):
            if((alpha[i] > 0 and alpha[i] < cost) or (label[i] > 0 and alpha[i] == cost) or (label[i] < 0 and alpha[i] == 0) ):
                tempF2[i] = devF[i];
            else:
                tempF2[i] = -float('Inf');
    
        #exit()
        #print "devF after sum is " + str(sum(devF))
        ...
        #print "iHigh is ", iHigh," iLow is " , iLow
        bHigh = devF[iHigh]
        bLow = devF[iLow]
