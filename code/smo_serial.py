...
##fjr read data
    fh = open("../dataset/heart_scale","r")
    label = []
    m = 0
    lines = fh.readlines()
    n = 13 #len(lines[0].split(' '))-1
    for l in lines:
        words = l.strip().split(' ')
        label.append(int(words[0]))
        #print(label[m])
        first = True
        valueVector = [0 for i in range(n)]
        for word in words:
            if first:
                first = False
                continue
            ...
            script = int(word[0:splitflag])
            #valueVector.append(value)
            valueVector[script-1] = value
        data.append(valueVector)
        m = m+1
    print data[0]
##fjr read data end
...
##        reads = infile.readline
##        line = reads[0:len(reads)-1]
    ## you yang read data return data & label
    tolerance = 1e-3
    epsilon = 1e-5
    ...
    print "iHigh ", iHigh, " iLow ", iLow
    pHigh = data[iHigh]
    pLow = data[iLow]
    print "pHigh is ", pHigh
    print "pLow is ", pLow
    ...
    while(bLow > bHigh + 2*tolerance):    
        for i in range(0, m):         
            ...
            devF[i] = devF[i] + alphaHighDiff * label[iHigh] * kernel(data[i], data[iHigh], n, gamma) + 
            alphaLowDiff * label[iLow] * kernel(data[i], data[iLow], n, gamma);
        ...
        bHigh = devF[iHigh]
        bLow = devF[iLow]
        # udpdate alpha_high and alpha_low according to Equ(6) and Equ(7)
